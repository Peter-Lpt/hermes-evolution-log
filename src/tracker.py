...
Usage:
    python3 tracker.py [--hermes-dir ~/.hermes] [--output ./data]
    python3 tracker.py --install  # First-time setup
"""
...
def track(hermes_dir, output_dir):
    ...
    data_file = os.path.join(output_dir, "evolution.json")
    snapshot_file = os.path.join(output_dir, "snapshots", "state.json")
    ...
    for name, info in current_skills.items():
        if name in prev_skills:
            ...
            if prev_skills:  # Only record new skills if baseline exists
                events.append({
                    "type": "skill_created",
                    "category": info["category"],
                    "timestamp": now,
    ...
    if prev_user_hash and user_hash and prev_user_hash != user_hash:
        events.append({
            "type": "user_modeling",
            "category": "用户建模",
            "timestamp": now,
    ...
    if prev_memory_hash and memory_hash and prev_memory_hash != memory_hash:
        events.append({
            "type": "memory_changed",
            "category": "长期记忆",
            "timestamp": now,
    ...
    evo = load_json(data_file, {"events": [], "skills_summary": {"used": {}, "unused": {}}})
    evo["events"].extend(events)

    # Split used / unused
    used_skills = {}
    unused_skills = {}
    for name, info in current_skills.items():
        count = usage.get(name, {}).get("count", 0) if isinstance(usage.get(name), dict) else 0
        entry = {
            "name": name,
        ...
        cat = info["category"]
        if count > 0:
            used_skills.setdefault(cat, []).append(entry)
        else:
            unused_skills.setdefault(cat, []).append(entry)
    ...
    used_count = sum(1 for s in current_skills.values()
                     if isinstance(usage.get(s.get("name", "")), dict)
                     and usage.get(s.get("name", ""), {}).get("count", 0) > 0)

    evo["stats"] = {
        "total_skills": len(current_skills),
        "used_skills": used_count,
        "unused_skills": len(current_skills) - used_count,
        "total_categories": len(set(i["category"] for i in current_skills.values())),
        "total_usage": sum(usage.get(n, {}).get("count", 0)
                          for n in current_skills if isinstance(usage.get(n), dict)),
        "last_scan": now,
    }

    save_json(data_file, evo)

    print(f"✅ 扫描完成: {len(current_skills)} 个技能, {len(events)} 个新事件")
    if events:
        for e in events:
            print(f"   → {e['type']}: {e['data'].get('skill_name', e['data'].get('description', ''))}")
    else:
        print("   无变化")
...
def main():
    ...
    parser.add_argument("--hermes-dir", default=os.environ.get("HERMES_DIR", DEFAULT_HERMES_DIR),
                        help="Hermes Agent home directory (default: ~/.hermes)")
    parser.add_argument("--output", default=os.environ.get("EVO_OUTPUT_DIR", DEFAULT_OUTPUT_DIR),
                        help="Output directory for evolution.json (default: ./data)")
    parser.add_argument("--install", action="store_true",
                        help="First-time setup: create directories and initial snapshot")
    args = parser.parse_args()

    if args.install:
        os.makedirs(args.output, exist_ok=True)
        os.makedirs(os.path.join(args.output, "snapshots"), exist_ok=True)
        print(f"📁 输出目录: {args.output}")

    track(args.hermes_dir, args.output)
